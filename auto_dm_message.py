...
message = {"content": "hey <user> join my telegram: https://t.me/tokens404"}
...
sleep_time = int(input(f"{Fore.RED}Timeout (5): {Fore.RESET}"))
...
def send_message(authorization, channel, msg, userID):
	...
	if response4.status_code == 200:
		print(f"{Fore.LIGHTGREEN_EX}[+] DM Sent to {userID} {Fore.RESET}({Fore.LIGHTBLACK_EX}{authorization[:50]}*******{Fore.RESET}){Fore.RESET}")
...
while True:
	try:
		response = requests.get(f"https://discord.com/api/v9/channels/{channel_id}/messages?limit=3", headers=headers, cookies=request_cookie(), timeout=15).json()
		for IDS in response:
			id_list.append(IDS['author']['id'])
		for id in id_list:
			if id not in open("sent.txt").read():
				file = open("sent.txt", 'a')
				print(f"{Fore.YELLOW}[!] {id} message has been found{Fore.RESET}")
				file.write(id + "\n")
				file.close()
				while True:
